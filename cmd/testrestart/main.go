...
func main() {
	...
	restartWorker := workers.NewRestartWorker(runtime, repo, logger)
	dispatcher := workers.NewDispatcher(q, 1)
	dispatcher.Register(jobs.JobTypeServerRestart, restartWorker.Handle)
	...
	job, err := jobs.New(jobs.JobTypeServerRestart, "test-provision-2", payload, 3)
	if err != nil {
	...
	fmt.Println("Restart job enqueued, waiting for server to reach Ready state...")
