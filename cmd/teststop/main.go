...
func main() {
	...
	stopWorker := workers.NewStopWorker(runtime, repo, logger)
	dispatcher := workers.NewDispatcher(q, 1)
	dispatcher.Register(jobs.JobTypeServerStop, stopWorker.Handle)
	...
	job, err := jobs.New(jobs.JobTypeServerStop, "test-provision-2", payload, 3)
	if err != nil {
